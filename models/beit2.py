...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, return_attention=False, return_qkv=False, image_atts=None):
        B, N, C = x.shape
        qkv_bias = None
...
class Block(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, return_attention=False, return_qkv=False, image_atts=None):
        ...
        if self.gamma_1 is None:
            x = x + self.drop_path(self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias, image_atts=image_atts))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias, image_atts=image_atts))
            x = x + self.drop_path(self.gamma_2 * self.mlp(self.norm2(x)))
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 use_mean_pooling=True, init_scale=0.001, local_attn_depth=-1,
                 num_masking_patches=75, min_num_patches=16):
        super().__init__()
        print("### local_attn_depth: ", local_attn_depth, flush=True)
        self.local_attn_depth = local_attn_depth  # to calculate region features
        self.depth = depth
        ...
        self.generator = MaskingGenerator(input_size=int(img_size/patch_size), num_masking_patches=num_masking_patches, min_num_patches=min_num_patches,)
        if use_abs_pos_emb:
    ...
    def forward_localattn(self, x, idx_to_group_img=None, image_atts=None):
        B, nc, w, h = x.shape
        ...
        for idx, blk in enumerate(self.blocks):
            if idx >= self.depth - self.local_attn_depth:
                if do_gather:
                    do_gather = False
                    x_bs = torch.gather(x, dim=0, index=idx_to_group_img.view(-1, 1, 1).expand(-1, x.shape[1], x.shape[2]))
                    x = torch.cat([x_bs, x], dim=0)

                x = blk(x, rel_pos_bias=rel_pos_bias, image_atts=image_atts_blk)
            else:
                x = blk(x, rel_pos_bias=rel_pos_bias, image_atts=None)

        x = self.fc_norm(self.norm(x))

        if idx_to_group_img is not None:
            bs = len(idx_to_group_img)
            outputs, outputs_fullatts = torch.split(x, [bs, x.size(0)-bs])
            return outputs, outputs_fullatts

        return x

    def forward_avgpool(self, x, idx_to_group_img=None, image_atts=None, do_mask=False):
        B, nc, w, h = x.shape
        x = self.patch_embed(x)
        batch_size, seq_len, _ = x.size()
        ...
        ids_mask = None
        if do_mask:
            # ids_mask = torch.full([batch_size, seq_len], 0.0)
            ids_mask = []
            for _ in range(B):
                mask = self.generator().flatten()
                ids_mask.append(torch.Tensor(mask.flatten()))
            # mask_prob = torch.full([batch_size, seq_len], 0.0)
            # ids_mask = torch.bernoulli(mask_prob).bool()
            mask_tokens = self.mask_token.expand(batch_size, seq_len, -1)
            ids_mask = torch.stack(ids_mask, 0).bool()
            w = ids_mask.unsqueeze(-1).type_as(mask_tokens)
        ...
        if idx_to_group_img is None:
            if do_mask:
                return torch.cat([x_cls.transpose(1, 2), x], dim=1), ids_mask
            else:
    ...
    def forward(self, x, idx_to_group_img=None, image_atts=None, do_mask=False):
        if self.local_attn_depth > 0:
            return self.forward_localattn(x, idx_to_group_img=idx_to_group_img, image_atts=image_atts)
        else:
            return self.forward_avgpool(x, idx_to_group_img=idx_to_group_img, image_atts=image_atts, do_mask=do_mask)
