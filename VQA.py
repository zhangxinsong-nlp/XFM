...
from models.model_generation import XFMForVQA
...
def main(args, config):
    ...
    load_vqa_pretrain = config.get('load_vqa_pretrain', False)
    model = XFMForVQA(config=config)
    model.load_pretrained(args.checkpoint, config, is_eval=args.evaluate or load_vqa_pretrain)
    ...
    write_to_hdfs = True if args.output_hdfs.startswith('hdfs') else False
    ...
    if args.evaluate:
        ...
        result_rpath = collect_result(vqa_result, 'vqa_eval', local_wdir=args.result_dir,
                                      ...
                                      write_to_hdfs=write_to_hdfs, save_result=True)
        ...
        if utils.is_main_process():
            ...
            print(f"### data {train_dataset_size}, batch size, {config['batch_size_train']} x {world_size} x {accumulate_steps}")
        ...
        # debug
        # vqa_result = evaluation(model, test_loader, tokenizer, device, config)
        # result = collect_result(vqa_result, 'vqa_zeroshot', local_wdir=args.result_dir,
        #                         hdfs_wdir=args.output_hdfs,
        #                         write_to_hdfs=write_to_hdfs, save_result=True)
