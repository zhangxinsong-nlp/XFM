...
from models.model_retrieval import XFMForRetrieval
...
def evaluation(model, data_loader, tokenizer, device, config):
    """
    X-Brain_base uses 12L text encoder for pre-ranking; then 12L cross encoder for ranking.
    (double computation compared to X-VLM_base)
    """
    model.eval()
    ...
    for i, sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, header)):
        ...
        output = model.get_cross_embeds(image_embeds=encoder_output, image_atts=encoder_att,
                                        text_ids=text_ids[topk_idx], text_atts=text_atts[topk_idx], text_embeds=text_feats[topk_idx])
    ...
    for i, sims in enumerate(metric_logger.log_every(sims_matrix[start:end], 50, header)):
        ...
        output = model.get_cross_embeds(image_embeds=encoder_output, image_atts=encoder_att,
                                        text_ids=text_ids[start + i].repeat(config['k_test'], 1),
...
def main(args, config):
    ...
    print(f"Creating model XFM for retrieval ", flush=True)
    RetrievalModel = XFMForRetrieval
    ...
    if args.evaluate:
        ...
        if utils.is_main_process():
            ...
            print(f"### data {train_dataset_size}, batch size, {config['batch_size_train']} x {world_size} x {accumulation_steps}")
