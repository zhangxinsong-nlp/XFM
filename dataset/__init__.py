...
from dataset.pretrain_dataset import build_tokenizer, ImageTextJsonDataset, RegionTextJsonDataset, TextJsonDataset, ImageJsonDataset
...
from dataset.captioning_dataset import coco_karpathy_train, coco_karpathy_train_scst, coco_karpathy_caption_eval
...
def create_dataset(dataset, config, evaluate=False):
    ...
    if dataset == 'pretrain':
        ...
        if ('train_file_regions' in config.keys()) and len(config['train_file_regions']):
        ...
        if ('train_file_aux' in config.keys()) and len(config['train_file_aux']):
            image_dataset_aux = ImageTextJsonDataset(config, config['train_file_aux'], rank=int(os.environ.get('RANK') or 0),
                                                   ...
                                                   repeat=True, transform=pretrain_transform, config_key='images_aux')
        else:
            image_dataset_aux = None

        if ('train_file_text' in config.keys()) and len(config['train_file_text']):
            text_dataset = TextJsonDataset(config, config['train_file_text'], rank=int(os.environ.get('RANK') or 0),
                                           world_size=int(os.environ.get('WORLD_SIZE') or 1), shuffle=True, repeat=True)
        else:
            text_dataset = None

        if ('train_file_web' in config.keys()) and len(config['train_file_web']):
            image_dataset_web = ImageTextJsonDataset(config, config['train_file_web'], rank=int(os.environ.get('RANK') or 0),
                                                   world_size=int(os.environ.get('WORLD_SIZE') or 1), shuffle=True,
                                                   repeat=True, transform=pretrain_transform, config_key='images_web')
        else:
            image_dataset_web = None

        if ('train_file_imagenet' in config.keys()) and len(config['train_file_imagenet']):
            image_dataset_imagenet = ImageJsonDataset(config, config['train_file_imagenet'], rank=int(os.environ.get('RANK') or 0),
                                                   world_size=int(os.environ.get('WORLD_SIZE') or 1), shuffle=True,
                                                   repeat=True, transform=pretrain_transform, config_key='images_imagenet')
        else:
            image_dataset_imagenet = None

        return image_dataset, region_dataset, text_dataset, image_dataset_aux, image_dataset_web, image_dataset_imagenet
